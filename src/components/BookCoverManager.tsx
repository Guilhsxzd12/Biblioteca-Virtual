...
export function BookCoverManager({bookId,title,mainCoverUrl}:{bookId:string;title:string;mainCoverUrl:string|null}){
  const [covers,setCovers]=useState<AltCover[]>([]);const [busy,setBusy]=useState(false);const [message,setMessage]=useState("");
  ...
  async function saveCover(coverUrl:string,label?:string){
    ...
    const data=await response.json();if(!response.ok)throw new Error(data.error||"Não foi possível adicionar a capa.");await load();
  }
  ...
  async function upload(file:File){
    ...
    try{const form=new FormData();form.append("file",file);const response=await fetch("/api/admin/covers",{method:"POST",body:form});const data=await response.json();if(!response.ok)throw new Error(data.error||"Não foi possível enviar a capa.");await saveCover(data.coverUrl,`Capa alternativa ${covers.length+1}`);setMessage("Capa alternativa adicionada.");}
    catch(error){setMessage(error instanceof Error?error.message:"Erro ao enviar capa.");}
    finally{setBusy(false);if(inputRef.current)inputRef.current.value="";}
  }

  async function addUrl(){if(!url.trim())return;setBusy(true);setMessage("");try{await saveCover(url.trim(),`Capa alternativa ${covers.length+1}`);setUrl("");setMessage("Capa alternativa adicionada.");}catch(error){setMessage(error instanceof Error?error.message:"Erro ao adicionar capa.");}finally{setBusy(false);}}
  ...
  return <section className="card panel cover-admin-panel">
    <div className="panel-title"><div><span className="eyebrow">CAPAS DO KINDLE</span><h2>Capas alternativas</h2><p className="muted">Somente capas deste livro. Elas aparecem como opções antes de baixar o EPUB.</p></div></div>
    <div className="admin-cover-grid">
      {mainCoverUrl&&<div className="admin-cover-item main"><img src={mainCoverUrl} alt={`Capa principal de ${title}`}/><strong>Capa principal</strong></div>}
      {covers.map((cover,index)=><div className="admin-cover-item" key={cover.id}><img src={cover.cover_url} alt={`Capa alternativa ${index+1}`}/><strong>{cover.label||`Alternativa ${index+1}`}</strong><button type="button" className="btn danger small" onClick={()=>void remove(cover.id)}>Remover</button></div>)}
    </div>
