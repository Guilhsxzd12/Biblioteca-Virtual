...
function date(value?:string|null){if(!value)return "—";return new Intl.DateTimeFormat("pt-BR",{dateStyle:"short",timeStyle:"short"}).format(new Date(value));}
...
export function SubscriptionsAdmin(){
  ...
  async function change(userId:string,action:"activate"|"cancel",days=30){setBusy(userId);setMessage("");try{const r=await fetch("/api/admin/subscriptions",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({userId,action,days})});const d=await r.json();if(!r.ok)throw new Error(d.error||"Não foi possível atualizar.");setMessage(action==="activate"?`Assinatura liberada por ${days} dias.`:"Assinatura cancelada.");await load();}catch(e){setMessage(e instanceof Error?e.message:"Erro ao atualizar.");}finally{setBusy(null);}}
  ...
  return <div className="stack" style={{gap:18}}>
    <section className="card panel"><div className="row wrap" style={{justifyContent:"space-between"}}><div><h2>Bot do Telegram</h2><p className="muted">O botão abaixo registra o webhook seguro no domínio oficial da Biblioteca.</p>{bot&&<p><b>@{bot.username||bot.first_name}</b> • token reconhecido</p>}</div><button className="btn" onClick={setupBot} disabled={botBusy}>{botBusy?"Ativando...":"Ativar / atualizar bot"}</button></div></section>
    {message&&<div className={`notice ${message.includes("liberada")||message.includes("ativado")?"success":""}`}>{message}</div>}
    <section className="card panel"><div className="row wrap" style={{justifyContent:"space-between",alignItems:"end"}}><div><h2>Assinaturas</h2><p className="muted">Confirme o pagamento manualmente e libere o acesso pelo @ do Telegram.</p></div><label style={{minWidth:260}}>Buscar usuário<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Nome, e-mail ou @telegram"/></label></div>
      {loading?<p className="muted">Carregando...</p>:<div className="table-list">{filtered.length?filtered.map(row=>{const active=isActive(row);return <div className="table-row" key={row.profile.id}><div><strong>{row.profile.full_name||row.profile.email||"Usuário"}</strong><div className="meta">{row.profile.email||"sem e-mail"} • {row.telegram?`@${row.telegram.username||row.telegram.first_name||row.telegram.telegram_user_id}`:"Telegram não vinculado"}</div><div className="meta">{active?`✅ Ativa até ${date(row.subscription?.active_until)}`:row.subscription?.status==="canceled"?"⛔ Cancelada":"⚪ Inativa"} • {row.profile.approved?"conta aprovada":"conta aguardando"}</div></div><div className="row wrap"><button className="btn" disabled={busy===row.profile.id} onClick={()=>change(row.profile.id,"activate",30)}>+30 dias</button><button className="btn secondary" disabled={busy===row.profile.id} onClick={()=>change(row.profile.id,"activate",90)}>+90 dias</button><button className="btn danger" disabled={busy===row.profile.id||!active} onClick={()=>change(row.profile.id,"cancel")}>Cancelar</button></div></div>}):<p className="muted">Nenhum usuário encontrado.</p>}</div>}
    </section>
  </div>;
}
