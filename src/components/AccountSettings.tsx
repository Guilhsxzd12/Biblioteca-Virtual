...
type Draft={fullName:string;email:string;username:string;currentPassword:string;password:string;confirmPassword:string};

export function AccountSettings({initialProfile}:{initialProfile:Profile}){
  const [profile,setProfile]=useState(initialProfile);
  const [draft,setDraft]=useState<Draft>({fullName:initialProfile.full_name||"",email:initialProfile.email||"",username:initialProfile.username||"",currentPassword:"",password:"",confirmPassword:""});
  ...
  async function submit(event:React.FormEvent){
    ...
    const emailChanged=draft.email.trim().toLowerCase()!==(profile.email||"").toLowerCase();
    if((emailChanged||draft.password)&&!draft.currentPassword){setMessage("Digite sua senha atual para confirmar alterações de e-mail ou senha.");return;}
    setBusy(true);
    try{
      const response=await fetch("/api/account",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({fullName:draft.fullName,email:draft.email,username:draft.username,currentPassword:draft.currentPassword,password:draft.password})});
      const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||"Não foi possível salvar.");
      setProfile(data.profile);setDraft(d=>({...d,fullName:data.profile.full_name||"",email:data.profile.email||"",username:data.profile.username||"",currentPassword:"",password:"",confirmPassword:""}));
  ...
  return <div className="account-settings-grid">
    <section className="card panel account-settings-card">
      <span className="eyebrow">DADOS DE ACESSO</span><h2>Minha conta</h2><p className="muted">Altere seus dados de login. Para trocar e-mail ou senha, confirme primeiro sua senha atual.</p>
      ...
      <form className="account-settings-form" onSubmit={submit}>
        <div className="account-form-grid">
          ...
          <label>Nome de usuário<input value={draft.username} onChange={e=>setDraft(d=>({...d,username:e.target.value}))} autoComplete="username" minLength={3} required/><small>Você também pode usar este nome no login.</small></label>
          <label className="account-span-2">E-mail<input type="email" value={draft.email} onChange={e=>setDraft(d=>({...d,email:e.target.value}))} autoComplete="email" required/></label>
        </div>
        <div className="account-password-box">
          ...
          <label>Senha atual<input type="password" value={draft.currentPassword} onChange={e=>setDraft(d=>({...d,currentPassword:e.target.value}))} autoComplete="current-password" placeholder="Obrigatória para trocar e-mail ou senha"/></label>
          <div className="account-form-grid">
