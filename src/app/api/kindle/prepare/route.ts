...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    return new NextResponse(result.bytes as BodyInit,{status:200,headers:{"content-type":"application/epub+zip","content-disposition":`attachment; filename="${safeName(result.fileName)}"`,`cache-control`:"private, no-store","x-book-title":encodeURIComponent(result.title)}});
  }catch(error){
