...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    const admin=createAdminSupabaseClient();const {data,error}=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{full_name:fullName}});
    if(error||!data.user)return NextResponse.json({error:error?.message||"Não foi possível criar o usuário."},{status:400});
    const now=new Date().toISOString();const {error:profileError}=await admin.from("profiles").upsert({id:data.user.id,email,full_name:fullName,role:"reader",approved:true,updated_at:now},{onConflict:"id"});
    if(profileError)return NextResponse.json({error:profileError.message},{status:400});
    await activateSubscription(data.user.id,viewer.user.id,"Conta criada após confirmação manual do pagamento");
    const {data:profile}=await admin.from("profiles").select("id,email,full_name,role,approved").eq("id",data.user.id).single();
...
export async function PATCH(request:NextRequest){
  ...
  try{
    ...
    const {data:profile}=await admin.from("profiles").select("id,email,full_name,role,approved").eq("id",id).single();
    return NextResponse.json({profile});
