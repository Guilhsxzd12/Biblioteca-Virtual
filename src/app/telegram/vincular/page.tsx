...
export default async function TelegramLinkPage({searchParams}:{searchParams:Promise<{code?:string}>}){
  ...
  const {error}=await admin.from("telegram_accounts").insert({user_id:viewer.user.id,telegram_user_id:link.telegram_user_id,chat_id:link.chat_id,username:link.username,first_name:link.first_name,updated_at:new Date().toISOString()});
  if(error)throw new Error(error.message);
  await admin.from("telegram_link_codes").update({used_at:new Date().toISOString()}).eq("code",code);
  try{await sendTelegramMessage(link.chat_id,"✅ <b>Conta vinculada!</b>\n\nSeu Telegram agora está conectado à Biblioteca Virtual. Use /status para consultar sua assinatura.");}catch(error){console.warn("[telegram-link] não foi possível confirmar no bot",error);}
