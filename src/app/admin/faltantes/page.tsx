...
export default async function MissingBooksPage(){
  ...
  const items=((books||[]) as Book[]).map(book=>{
    ...
    const missing:("pdf"|"epub")[]=[];if(!hasPdf)missing.push("pdf");if(!hasEpub)missing.push("epub");
    return {book,missing};
  }).filter(item=>item.missing.length>0);
  return <AppShell><main className="container admin-page">
    <div className="page-head"><div><span className="eyebrow">PAINEL ADMIN</span><h1>Faltantes</h1><p>Livros que ainda não possuem PDF ou EPUB.</p></div><Link className="btn ghost" href="/admin">← Voltar ao painel</Link></div>
