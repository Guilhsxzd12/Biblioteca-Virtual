...
function normalizeLanguage(value?:string|null){
  const raw=(value||"pt-BR").trim();
  return raw||"pt-BR";
}

function coverDriveId(url?:string|null){
  if(!url)return null;
...
export async function loadCoverBytes(url?:string|null):Promise<CoverData>{
  ...
  const driveId=coverDriveId(url);
  let response:Response;
  if(driveId)response=await fetchDriveFile(driveId);
  else{
    if(!/^https?:\/\//i.test(url))throw new Error("Capa inválida para o Kindle.");
...
function pageParagraphs(items:any[]){
  ...
  for(const item of items){
    ...
    if(item?.hasEOL&&current.trim()){paragraphs.push(current.trim());current="";}
  }
...
export async function buildEpubFromPdf(pdfBytes:Uint8Array,meta:EpubMeta){
  ...
  const chapters:{name:string;label:string;html:string}[]=[];
  let extractedChars=0;
  const pagesPerChapter=10;

  for(let start=1;start<=pdf.numPages;start+=pagesPerChapter){
    const end=Math.min(pdf.numPages,start+pagesPerChapter-1);
    const body:string[]=[];
    for(let pageNumber=start;pageNumber<=end;pageNumber++){
      const page=await pdf.getPage(pageNumber);
      const content=await page.getTextContent();
      const paragraphs=pageParagraphs(content.items as any[]);
      const pageText=paragraphs.join(" ");
      extractedChars+=pageText.length;
      if(paragraphs.length){
        body.push(`<section class="pdf-page" data-page="${pageNumber}"><span class="page-number">${pageNumber}</span>${paragraphs.map(p=>`<p>${escapeXml(p)}</p>`).join("")}</section>`);
      }
      page.cleanup();
    }
    const index=Math.floor((start-1)/pagesPerChapter)+1;
    chapters.push({name:`text/chapter-${String(index).padStart(3,"0")}.xhtml`,label:`Páginas ${start}–${end}`,html:body.join("\n")||`<section class="pdf-page"><p>Página sem texto extraível.</p></section>`});
  }

  if(extractedChars<Math.max(300,pdf.numPages*25)){
    await pdf.destroy();
    throw new Error("Este PDF parece ser escaneado ou não possui texto suficiente para criar um EPUB de boa qualidade. O PDF continua disponível e pode ser enviado diretamente ao Kindle.");
  }

  const cover=await loadCoverBytes(meta.coverUrl);
  const zip=new JSZip();
  ...
  const navItems=chapters.map((ch,i)=>`<li><a href="${ch.name}">${escapeXml(ch.label)}</a></li>`).join("");
  ...
  const manifest=chapters.map((ch,i)=>`<item id="ch${i+1}" href="${ch.name}" media-type="application/xhtml+xml"/>`).join("");
...
export async function replaceEpubCover(epubBytes:Uint8Array,coverUrl:string){
  const cover=await loadCoverBytes(coverUrl);
  const zip=await JSZip.loadAsync(epubBytes);
  const opfFile=zip.file("OEBPS/content.opf");
  const coverEntry=zip.file("OEBPS/images/cover");
  ...
  opf=opf.replace(/(<item id="cover-image" href="images\/cover" media-type=")[^"]+(" properties="cover-image"\/>)/,`$1${cover.mimeType}$2`);
