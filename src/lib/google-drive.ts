...
export async function getGoogleAccessToken(){
  ...
  const {clientId,clientSecret}=googleConfig();
  const response=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body:new URLSearchParams({client_id:clientId,client_secret:clientSecret,refresh_token:data.refresh_token,grant_type:"refresh_token"}),cache:"no-store"});
...
async function findOrCreateChildFolder(token:string,name:string,parentId:string){
  ...
  const created=await driveJson<{id:string}>(token,"https://www.googleapis.com/drive/v3/files?fields=id",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name,mimeType:"application/vnd.google-apps.folder",parents:[parentId]})});
  return created.id;
}
...
export async function findOrCreateRootFolder(token:string){
  ...
  if(result.files?.[0]?.id)return result.files[0].id;
  const created=await driveJson<{id:string}>(token,"https://www.googleapis.com/drive/v3/files?fields=id",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:"BIBLIOTECA VIRTUAL",mimeType:"application/vnd.google-apps.folder"})});
  return created.id;
}
...
async function findOrCreateUserFolder(token:string,userId:string,kind:"EBOOKS"|"CAPAS"){
  const root=await findOrCreateRootFolder(token);const users=await findOrCreateChildFolder(token,"USUARIOS",root);const user=await findOrCreateChildFolder(token,userId,users);return findOrCreateChildFolder(token,kind,user);
}
...
async function startResumableUpload(token:string,p:{fileName:string;mimeType:string;fileSize:number;folderId:string}){
  ...
  const uploadUrl=response.headers.get("location");if(!uploadUrl)throw new Error("Google Drive não retornou a sessão de upload.");return uploadUrl;
}
...
async function uploadFileToFolder(file:File,folderId:string){
  const token=await getGoogleAccessToken();const safeBase=file.name.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9._-]+/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")||"arquivo";const fileName=`${Date.now()}-${safeBase}`;const mimeType=file.type||"application/octet-stream";const uploadUrl=await startResumableUpload(token,{fileName,mimeType,fileSize:file.size,folderId});const response=await fetch(uploadUrl,{method:"PUT",headers:{"content-type":mimeType,"content-length":String(file.size)},body:await file.arrayBuffer()});
  if(!response.ok){const text=await response.text();throw new Error(`Upload falhou (${response.status}): ${text.slice(0,180)}`);}
  const uploaded=await response.json() as {id:string;name:string;mimeType:string};if(!uploaded.id)throw new Error("Google Drive não retornou o ID do arquivo.");return uploaded;
}
...
export async function fetchDriveFile(fileId:string,range?:string|null){
  const token=await getGoogleAccessToken();const response=await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media`,{headers:{authorization:`Bearer ${token}`,...(range?{range}:{})},cache:"no-store"});if(!response.ok&&response.status!==206)throw new Error(`Falha ao ler arquivo do Drive (${response.status}).`);return response;
}
