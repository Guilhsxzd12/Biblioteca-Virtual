...
export function getSiteOrigin(fallback?:string){
  return (process.env.NEXT_PUBLIC_SITE_URL?.trim()||DEFAULT_SITE_ORIGIN||fallback||"").replace(/\/$/,"");
}

export function makeGoogleAuthUrl(_origin:string,state:string){
  const {clientId}=googleConfig();
  const origin=getSiteOrigin(_origin);
...
export async function exchangeGoogleCode(code:string,_origin:string){
  ...
  if(!response.ok){
    ...
    throw new Error(`Falha ao conectar Google Drive (${response.status}): ${text.slice(0,160)}`);
  }
...
async function readRefreshToken(){
  ...
  try{
    ...
    if(error)throw new Error(`Supabase app_integrations: ${error.message}`);
    if(data?.refresh_token)return data.refresh_token as string;
  }catch(error){
    if(envToken){
      console.warn("[drive] usando refresh token de contingência",{reason:error instanceof Error?error.message:"erro desconhecido"});
      return envToken;
    }
    throw error;
  }
...
export async function getGoogleAccessToken(){
  ...
  if(!response.ok){
    const text=await response.text();
    console.error("[drive] refresh token recusado",{status:response.status,body:text.slice(0,180)});
    throw new Error(`Não foi possível renovar o acesso ao Drive (${response.status}).`);
  }
  const json=await response.json() as {access_token?:string};
  if(!json.access_token)throw new Error("Google não retornou access token.");
...
async function driveJson<T>(token:string,url:string,init?:RequestInit){
  ...
  if(!response.ok){const text=await response.text();throw new Error(`Google Drive ${response.status}: ${text.slice(0,300)}`);}
...
export async function findOrCreateRootFolder(token:string){
  ...
  if(result.files?.[0]?.id)return result.files[0].id;
  const created=await driveJson<{id:string}>(token,"https://www.googleapis.com/drive/v3/files?fields=id",{
    method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:"BIBLIOTECA VIRTUAL",mimeType:"application/vnd.google-apps.folder"})
  });
  return created.id;
}
...
async function findOrCreateUserFolder(token:string,userId:string,kind:"EBOOKS"|"CAPAS"|"KINDLE"){
...
async function findOrCreateCatalogKindleFolder(token:string,letter:string){
  const root=await findOrCreateRootFolder(token);
  const kindle=await findOrCreateChildFolder(token,"KINDLE",root);
  const safe=/^[A-Z]$/.test(letter)?letter:"#";
  return findOrCreateChildFolder(token,safe,kindle);
}
...
async function startResumableUpload(token:string,p:{fileName:string;mimeType:string;fileSize:number;folderId:string}){
  const response=await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&fields=id,name,mimeType,webViewLink",{
    method:"POST",
    headers:{authorization:`Bearer ${token}`,"content-type":"application/json; charset=UTF-8","x-upload-content-type":p.mimeType||"application/octet-stream","x-upload-content-length":String(p.fileSize)},
    body:JSON.stringify({name:p.fileName,mimeType:p.mimeType||"application/octet-stream",parents:[p.folderId]})
  });
  if(!response.ok){const text=await response.text();throw new Error(`Não foi possível iniciar o upload no Drive (${response.status}): ${text.slice(0,160)}`);}
...
export async function createResumableUpload(p:{fileName:string;mimeType:string;fileSize:number;letter:string}){
  const token=await getGoogleAccessToken();const folderId=await findOrCreateLetterFolder(token,p.letter);const uploadUrl=await startResumableUpload(token,{...p,folderId});return {uploadUrl,folderId};
}
export async function createUserBookResumableUpload(p:{userId:string;fileName:string;mimeType:string;fileSize:number}){
  const token=await getGoogleAccessToken();const folderId=await findOrCreateUserFolder(token,p.userId,"EBOOKS");const uploadUrl=await startResumableUpload(token,{fileName:p.fileName,mimeType:p.mimeType,fileSize:p.fileSize,folderId});return {uploadUrl,folderId};
}

async function uploadBytesToFolder(bytes:Uint8Array,fileName:string,mimeType:string,folderId:string){
  const token=await getGoogleAccessToken();
  const safe=safeFileName(fileName);
  const uploadUrl=await startResumableUpload(token,{fileName:safe,mimeType,fileSize:bytes.byteLength,folderId});
  const response=await fetch(uploadUrl,{method:"PUT",headers:{"content-type":mimeType,"content-length":String(bytes.byteLength)},body:bytes});
  if(!response.ok){const text=await response.text();throw new Error(`Upload falhou (${response.status}): ${text.slice(0,180)}`);}
...
async function uploadFileToFolder(file:File,folderId:string){
  const safeBase=safeFileName(file.name);const bytes=new Uint8Array(await file.arrayBuffer());
  return uploadBytesToFolder(bytes,`${Date.now()}-${safeBase}`,file.type||"application/octet-stream",folderId);
}
...
export async function fetchDriveFile(fileId:string,range?:string|null){
  const token=await getGoogleAccessToken();
  const response=await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media`,{headers:{authorization:`Bearer ${token}`,...(range?{range}:{})},cache:"no-store"});
  if(!response.ok&&response.status!==206){const text=await response.text();throw new Error(`Falha ao ler arquivo do Drive (${response.status}): ${text.slice(0,180)}`);}
  return response;
}

export async function deleteDriveFile(fileId:string){
  const token=await getGoogleAccessToken();
  const response=await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}`,{method:"DELETE",headers:{authorization:`Bearer ${token}`},cache:"no-store"});
  if(!response.ok&&response.status!==404){const text=await response.text();throw new Error(`Não foi possível excluir do Drive (${response.status}): ${text.slice(0,180)}`);}
}
